...
export default function StartHerePage() {
  const onboardingSteps = [
    { id: "posture", label: "Posture", href: "#start-posture", state: "Now" },
    { id: "situation", label: "Situation", href: "#start-situation", state: "Next" },
    { id: "toolkit", label: "Toolkit", href: "#start-toolkit", state: "Then" },
  ] as const;

  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-8 sm:px-6 sm:py-10">
      <section className="weather-panel space-y-4 px-6 py-6">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Onboarding overview</p>
        <h1 className="text-2xl font-semibold text-slate-100 sm:text-3xl">Posture, situation, and toolkit at a glance.</h1>
        <div className="grid gap-3 rounded-2xl border border-slate-800/85 bg-slate-950/55 p-3 sm:grid-cols-3">
          {onboardingSteps.map((step, index) => (
            ...
            >
              <span className="inline-flex items-center gap-2">
                <span className="flex h-7 w-7 items-center justify-center rounded-full border border-slate-700/80 text-xs font-semibold text-slate-200">
                  {index + 1}
                </span>
                <span className="font-semibold">{step.label}</span>
              </span>
              <span className="rounded-full border border-slate-700/75 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-300">
                {step.state}
              </span>
            </a>
          ))}
