...
| Next.js | 15.5.12 | 16.1.x | Pending Wave 1 spike | App Router and route handler behavior should be validated before upgrading to 16.x. | Next.js 16 blog + upgrade guide |
...
| ESLint | 8.57.0 (`eslint` CLI) | 9.x | Deferred after framework migration | Flat config transition and compatibility with `eslint-config-next`. | ESLint 9 release notes |
| Zod | 3.25.76 | 4.x | Deferred after framework migration | API changes may require schema migration in validators/tests. | Zod v4 migration docs |
